...
def main():
    ...
    with dai.Device(pipeline) as device:
        ...
        while True:
            ...
            left_circles = cv2.HoughCircles(left_denoised, cv2.HOUGH_GRADIENT, dp=1, minDist=100,
                                            param1=50, param2=30, minRadius=MIN_RADIUS, maxRadius=MAX_RADIUS)

            right_circles = cv2.HoughCircles(right_denoised, cv2.HOUGH_GRADIENT, dp=1, minDist=100,
                                             param1=50, param2=30, minRadius=MIN_RADIUS, maxRadius=MAX_RADIUS)

            # Draw circles on the frames and calculate ball positions and distances
            if left_circles is not None:
                ...
                for (x, y, r) in left_circles:
                    ...
                    print("Left Ball - Distance: {} mm, Position: ({}, {})".format(ball_distance, ball_position[0], ball_position[1]))
            ...
            if right_circles is not None:
                ...
                for (x, y, r) in right_circles:
                    ...
                    ball_distance = calculate_distance(r)
                    ball_position = calculate_position(x, y)
                    print("Right Ball - Distance: {} mm, Position: ({}, {})".format(ball_distance, ball_position[0], ball_position[1]))
...
def calculate_distance(radius):
    return CALIBRATED_DISTANCE * (CALIBRATED_RADIUS / radius)
